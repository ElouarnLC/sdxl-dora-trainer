#!/bin/bash
# DoRA training on vitraux dataset with DSPO evaluation

# Set paths
DATASET_PATH="datasets/vitraux"
REWARD_MODEL="outputs/full_enhanced_final/best/model.pt"
OUTPUT_DIR="outputs/dora_vitraux_training"

# Training parameters
BATCH_SIZE=2
NUM_STEPS=2000
RANK=16
LEARNING_RATE=5e-5
EVAL_EVERY=500
SAVE_EVERY=500

echo "🚀 DoRA Training on Vitraux Dataset with DSPO Evaluation"
echo "======================================================="
echo "📁 Dataset: $DATASET_PATH"
echo "🏆 Reward Model: $REWARD_MODEL"
echo "💾 Output: $OUTPUT_DIR"
echo "📊 Parameters: batch_size=$BATCH_SIZE, steps=$NUM_STEPS, rank=$RANK"
echo "🔍 DSPO Evaluation: every $EVAL_EVERY steps"
echo ""

# Run training
python scripts/train_dora_vitraux.py \
    --dataset-path "$DATASET_PATH" \
    --reward-model "$REWARD_MODEL" \
    --output-dir "$OUTPUT_DIR" \
    --batch-size $BATCH_SIZE \
    --num-steps $NUM_STEPS \
    --rank $RANK \
    --lr $LEARNING_RATE \
    --eval-every $EVAL_EVERY \
    --save-every $SAVE_EVERY

echo ""
echo "✅ Training completed!"
echo "📊 Check DSPO results: $OUTPUT_DIR/dspo_evaluation_results.csv"
echo "🏆 Best model: $OUTPUT_DIR/best_model"
echo "📁 Final model: $OUTPUT_DIR/final_model"
